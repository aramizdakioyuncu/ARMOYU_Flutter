...
class _EventStatePage extends State<EventPage> {
  ...
  Future<void> fetchparticipantList(eventID) async {
    ...
    setstatefunction();
    FunctionsEvent f =
        FunctionsEvent(currentUser: widget.currentUserAccounts.user);
    ...
    for (var element in response["icerik"]["participant_players"]) {
      if (element["player_ID"] == widget.currentUserAccounts.user.userID) {
        didijoin = true;
        setstatefunction();
      }
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
                            ...
                            builder: (context) => MediaViewer(
                              ...
                              media: [
                                Media(
                                  mediaID: 0,
                                  mediaURL: MediaURL(
                                    bigURL: widget.event.detailImage.toString(),
                                    normalURL:
                                        widget.event.detailImage.toString(),
                                    minURL: widget.event.detailImage.toString(),
                                  ),
                                ),
                              ],
                              initialIndex: 0,
          ...
          SliverToBoxAdapter(
            child: !fetchParticipantProccess
                ? Column(
                    children: [
                      Visibility(
                        visible: widget.event.participantgroupsList!.isNotEmpty,
                        child: SizedBox(
                          height: 600,
                          child: ListView.separated(
                            separatorBuilder: (context, index) =>
                      ...
                      Visibility(
                        visible: widget.event.participantpeopleList!.isNotEmpty,
                        child: SizedBox(
                          height: 350,
                          child: ListView.separated(
                            scrollDirection: Axis.vertical,
                              ...
                              return ListTile(
                                ...
                                onTap: () {
                                  ...
                                  functions.pushProfilePage(
                                    ...
                                    User(
                                      userID: widget.event
                                          .participantpeopleList![index].userID,
                                    ),
                                    ScrollController(),
                                  );
                                },
